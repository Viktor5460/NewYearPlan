...
function updateSliderFromScroll() {
    ...
    // Вычисляем текущую позицию скролла (центр видимой области)
    ...
    const centerTime = new Date(minTime.getTime() + minutesFromStart * 60 * 1000);
    
    // Вычисляем процент для ползунка
    const percent = ((centerTime - minTime) / (maxTime - minTime)) * 100;
    const clampedPercent = Math.max(0, Math.min(100, percent));
    
    // Обновляем ползунок без привязки к меткам (плавное следование)
    currentSliderValue = clampedPercent;
    const slider = document.getElementById('time-scale-slider');
    if (slider) {
        slider.style.top = `${clampedPercent}%`;
    }
...
function calculateTimeRange() {
    ...
    maxTime = new Date(Math.max(...times.map((t, i) => {
        const endTime = parseDateTimeStr(allLettersData[i].end_datetime_str);
...
function renderLetters() {
    ...
    groupedLetters.forEach((group) => {
        ...
        if (letters.length === 1) {
            ...
            const height = Math.max(MIN_LETTER_HEIGHT, duration * PIXELS_PER_MINUTE);
            const rotation = (Math.random() - 0.5) * 2 * RANDOM_ROTATION_MAX;
    ...
    // Высота должна соответствовать полному временному диапазону
    const totalHeight = totalMinutes * PIXELS_PER_MINUTE * LETTER_SCALE;
    tableSurface.style.minHeight = `${Math.max(maxBottom, totalHeight) + 200}px`;
}
...
function renderTimeScale() {
    ...
    // Добавляем серые метки для центров каждого письма (включая специальные)
    allLettersData.forEach(letter => {
        ...
        if (startTime && endTime && startTime >= minTime && endTime <= maxTime) {
            // Вычисляем центр письма: начало + половина продолжительности
            // Письмо позиционируется так: верх на startTime, высота = duration * PIXELS_PER_MINUTE
            // Визуальный центр находится на startTime + duration/2
            const durationMinutes = letter.duration_minutes || 30;
            const centerTime = new Date(startTime.getTime() + (durationMinutes * 60 * 1000) / 2);
            
            if (centerTime >= minTime && centerTime <= maxTime) {
                const percent = ((centerTime - minTime) / (maxTime - minTime)) * 100;
...
function scrollToTime(dt) {
    ...
    if (closestLetter) {
        ...
        const containerHeight = tableContainer.clientHeight;
        const scrollTop = centerPosition - (containerHeight / 2);
        ...
        const top = minutesFromStart * PIXELS_PER_MINUTE * LETTER_SCALE;
        const containerHeight = tableContainer.clientHeight;
        tableContainer.scrollTop = Math.max(0, top - (containerHeight / 2));
    }
}
...
function applyInspectionMode() {
    ...
    // На мобильных устройствах уменьшаем ширину левой колонки в 1.5 раза
    let leftColumnWidth = 450; // Увеличено в 1.5 раза (было 300)
    if (window.innerWidth <= 480) {
        leftColumnWidth = 200; // Для очень маленьких экранов
    } else if (window.innerWidth <= 768) {
        leftColumnWidth = 300; // Для мобильных устройств (450 / 1.5)
    }
